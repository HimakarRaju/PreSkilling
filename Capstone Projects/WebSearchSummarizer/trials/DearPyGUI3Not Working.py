    ...
    def worker_thread():
        try:
            ...
            if search_results:
                ...
                # Show the response window and provide feedback (using async callbacks)
                dpg.run_async_callback(dpg.configure_item, "response_window", show=True)
                dpg.run_async_callback(dpg.set_value, "response_window_text",
                                       f"Generating response... (ETA: ~{estimated_time} seconds)")

                # Add a spinner within the 'response_window' (using async callbacks)
                def add_spinner():
                    with dpg.group(parent="response_window_group", horizontal=True):
                        spinner_id = dpg.add_loading_indicator(color=(255, 255, 255))
                        dpg.add_text("Generating response...")
                    return spinner_id

                spinner_id = dpg.run_async_callback(add_spinner)

                gemma_response = generate_response(prompt, model, tokenizer, device)

                # Remove the spinner after response generation (using async callbacks)
                dpg.run_async_callback(dpg.delete_item, spinner_id)
                dpg.run_async_callback(dpg.set_value, "response_window_text", gemma_response)

                docx_filename = f"ai_response_{int(time.time())}.docx"
                create_docx(question, gemma_response, docx_filename)

            else:
                dpg.run_async_callback(dpg.set_value, "response_text", "No answer found.")

            dpg.run_async_callback(dpg.configure_item, "ask_button", enabled=True)

        except Exception as e:
            print(f"Error in worker thread: {e}")
            dpg.run_async_callback(dpg.set_value, "response_text", "An error occurred.")
            dpg.run_async_callback(dpg.configure_item, "ask_button", enabled=True)
...
with dpg.window(label="AI Assistant", width=780, height=580, pos=(10, 10), tag="main_window"):
    with dpg.group():  # Create a group for the title
        dpg.bind_font(title_font)  # Apply title_font to the group
        dpg.add_text("AI Assistant")  # Title with larger font
    ...
    dpg.add_radio_button(items=["Detail and Analysis", "Explore Topics", "Structured Response", "Comprehensive"],
                         tag="prompt_choice", default_value=0, horizontal=True)
