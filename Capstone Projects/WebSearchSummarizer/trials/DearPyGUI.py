...
def ask_question(sender, app_data):
    ...
    dpg.set_value("response_text", "Scraping Google...")
    search_results = scrape_google_search(question)

    if search_results:
        choice = dpg.get_value("prompt_choice")  # No need to add 1
        prompt = generate_prompt(choice, search_results)

        estimated_time = estimate_response_time(prompt, model, tokenizer, device)
        dpg.set_value("response_text", f"Generating response... (ETA: ~{estimated_time} seconds)")

        # Add a spinner within the 'response_text' group
        dpg.set_value("response_text", "")  # Clear previous response
        with dpg.group(parent="response_text_group", horizontal=True):
            dpg.add_loading_indicator(color=(255, 255, 255))
            dpg.add_text("Generating response...")

        gemma_response = generate_response(prompt, model, tokenizer, device)
        dpg.set_value("response_text", gemma_response)

        docx_filename = f"ai_response_{int(time.time())}.docx"
        create_docx(question, gemma_response, docx_filename)

    else:
        dpg.set_value("response_text", "No answer found.")
...
with dpg.window(label="AI Assistant", width=780, height=580, pos=(10, 10), tag="main_window"):
    ...
    dpg.add_text("AI Assistant Application", color=(255, 255, 255))
    dpg.bind_font(default_font)  # Reset to default font after title
