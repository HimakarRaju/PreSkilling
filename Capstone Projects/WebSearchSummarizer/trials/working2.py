...
def generate_response(prompt, model, tokenizer, device):
    ...
    try:
        ...
        if answer:
            answer_cleaned = answer.replace(prompt, "")
...
@measure_time
def scrape_google_search(query, num_results=5):  # Reduced default num_results
    ...
    try:
        ...
        for i in range(min(num_results, len(result_divs))):
...
def process_question(question, model, tokenizer, device):
    """Handles the processing of a single question."""
...
def main():
    ...
    while True:
        ...
        # Using a thread for each question
        thread = threading.Thread(target=process_question, args=(question, model, tokenizer, device))
        thread.start()
