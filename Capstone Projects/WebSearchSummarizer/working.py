...
def load_model():
    ...
    #     print("If gpu is present try installing CUDA using \n"
...
#
#         # Remove everything before "**Response:**" or "**Analysis:**" if they exist
#         if "**Response:**" in answer:
#             answer = answer.split("**Response:**", 1)[1].strip()
#         elif "**Analysis:**" in answer:
#             answer = answer.split("**Analysis:**", 1)[1].strip()
#
...
def generate_response(prompt, model, tokenizer, device):
    ...
    try:
        generator = pipeline("text-generation", model=model, tokenizer=tokenizer, device=device)
        with tqdm(total=1, desc="Generating response") as pbar:
            outputs = generator(prompt, max_new_tokens=1000)
            pbar.update(1)
...
paragraph = document.add_paragraph("This is a paragraph with a custom style.", style='CustomStyle')

# Save the document
document.save('custom_styled_paragraph.docx')
"""