...
def suggest_optimization(df):
    ...
    if numeric_columns:
        ...
        if len(numeric_columns) > 1:
            ...
            )  # Pass the last numeric column as the target
        ...
        if categorical_columns:
            ...
            tree_report = run_decision_tree(df, categorical_columns[0])
            report += "\nDecision Tree Classifier Report:\n" + tree_report

            # Random Forest
            report += "     * Random Forest Classifier: A robust ensemble method.\n"
            forest_report = run_random_forest(df, categorical_columns[0])
