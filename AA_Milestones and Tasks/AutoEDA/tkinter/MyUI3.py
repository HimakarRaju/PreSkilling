...
class EDAApp:
    def __init__(self, root):
        ...
        # Create a frame for resource usage on the top-left below the close button
        ...
        # Load dataset button in the menu
        self.load_button = ttk.Button(
            root, text="Load Dataset(s)", command=self.load_datasets
        )
        self.load_button.grid(row=0, column=1, padx=5, pady=5, sticky="e")

        # IPython toggle button
        self.toggle_ipython_button = ttk.Button(
            root, text="Toggle IPython", command=self.toggle_ipython_panel
        )
        self.toggle_ipython_button.grid(row=0, column=2, padx=5, pady=5, sticky="e")
        ...
        # Status bar with a scroll bar for updates
        ...
        self.data_view_frame.grid(row=1, column=1, columnspan=2, sticky="nsew")
        ...
        # IPython interactive shell at the bottom (hidden initially)
        ...
        self.ipython_output.grid_remove()  # Initially hidden
    ...
    def setup_ipython_redirection(self):
        ...
        self.shell.push({"print": lambda msg: self.display_in_data_view(msg)})
    ...
    def toggle_ipython_panel(self):
        """Toggle the visibility of the IPython panel."""
        if self.ipython_output.winfo_ismapped():
            self.ipython_output.grid_remove()
            self.log_status("IPython panel hidden.")
        else:
            self.ipython_output.grid()
            self.log_status("IPython panel shown.")
            self.shell.mainloop()

    def display_in_data_view(self, result):
        """Displays IPython operations results in the main data view."""
        self.data_view.config(state=tk.NORMAL)  # Allow changes
        self.data_view.delete(1.0, tk.END)  # Clear previous content
        self.data_view.insert(tk.END, str(result))  # Insert result
        self.data_view.config(state=tk.DISABLED)  # Make it read-only again
        self.log_status(f"Displayed IPython result in main data view.")
